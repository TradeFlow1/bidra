...
- [ ] CATS-01: Categories are unclear. Replace with full Category + Subcategory taxonomy (eBay/Gumtree style, original copy). Apply consistently on: Home category chips, /listings filters, /sell/new dropdown.
- [ ] PAY-01: Remove any hardcoded support@bidra.com.au usage in user-facing flows/legal copy. Replace with neutral wording (no email) OR configurable seller contact.
- [ ] PAY-02: PayID/Osko payment: allow sellers to set PayID details in Account Settings (optional). After a sale: if seller has PayID show instructions; if not, show “Contact seller to arrange payment.”
- [ ] ACCT-01: Consolidate /dashboard and /account into one simple “My Account” hub (keep routes, but make one redirect or one canonical UI).

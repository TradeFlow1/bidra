...
export default function BrandLogo({ className, priority = false, variant = "full", tone = "dark" }: BrandLogoProps) {
  if (variant === "symbol") {
    return <Image src="/brand/bidra-child-drawing-mark.svg" alt="Bidra" width={96} height={96} priority={priority} unoptimized className={className || "h-10 w-10 object-contain"} />;
  }
  ...
  return (
    <span className={className || "inline-flex items-center gap-3"}>
      <Image src="/brand/bidra-child-drawing-mark.svg" alt="" width={52} height={52} priority={priority} unoptimized className="h-12 w-12 shrink-0 object-contain" />

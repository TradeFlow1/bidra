    ...
    <>
      <section className="md:hidden">
        <div className="rounded-[26px] border border-[#C7D2FE] bg-[linear-gradient(135deg,#F8FBFC_0%,#EEF2FF_100%)] p-5 shadow-[0_18px_50px_rgba(32,75,140,0.10)]">
          <div className="inline-flex rounded-full border border-[#C7D2FE] bg-white px-3 py-2 text-[11px] font-black leading-none text-[#4F46E5] shadow-sm hover:bg-[#F5F3FF]">
            Local marketplace
          </div>
          ...
          <div className="mt-5 grid gap-3">
            <Link href="/listings" className="bd-hero-primary-button flex h-12 items-center justify-center rounded-2xl border border-[#4F46E5] bg-white px-5 text-sm font-black text-[#4F46E5] shadow-sm hover:bg-[#F5F3FF]">
              Browse listings
            </Link>
            <Link href={sellHref} className="flex h-12 items-center justify-center rounded-2xl border border-[#C7D2FE] bg-white px-5 text-sm font-black text-[#4F46E5] shadow-sm hover:bg-[#F5F3FF]">
              Sell an item
            </Link>
          </div>

          <div className="mt-5 grid grid-cols-3 gap-2">
            <div className="rounded-2xl bg-white/85 px-2 py-3 text-center shadow-sm ring-1 ring-[#C7D2FE] hover:bg-[#F5F3FF]">
      ...
      <section className="relative hidden overflow-hidden rounded-[28px] border border-[#C7D2FE] bg-[linear-gradient(135deg,#F6F8FB_0%,#EEF2FF_100%)] p-8 shadow-[0_22px_70px_rgba(32,75,140,0.11)] md:block lg:p-10">
        <div className="pointer-events-none absolute -right-20 -top-24 h-80 w-80 rounded-full bg-white/60 blur-2xl hover:bg-[#F5F3FF]" />
        <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(520px,0.95fr)] lg:items-center">
          <div>
            <div className="inline-flex rounded-full border border-[#C7D2FE] bg-white px-4 py-2 text-xs font-black text-[#4F46E5] shadow-sm hover:bg-[#F5F3FF]">
            ...
            <div className="mt-5 flex gap-3">
              <Link href="/listings" className="bd-hero-primary-button inline-flex h-12 items-center justify-center rounded-2xl border border-[#4F46E5] bg-white px-6 text-sm font-black text-[#4F46E5] shadow-sm hover:bg-[#F5F3FF]">
                Browse listings
              </Link>
              <Link href={sellHref} className="inline-flex h-12 items-center justify-center rounded-2xl border border-[#C7D2FE] bg-white px-6 text-sm font-black text-[#4F46E5] shadow-sm hover:bg-[#F5F3FF]">
                Sell an item
              </Link>
            </div>
    ...
    <div className="relative hidden grid-cols-2 gap-4 sm:grid lg:grid lg:min-h-[430px]">
      ...
      {heroListings.map((item, index) => (
        ...
        >
          ...
          <span className="absolute left-3 top-3 rounded-full bg-white px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-[#4F46E5] shadow-lg ring-1 ring-[#C7D2FE] hover:bg-[#F5F3FF]">
            {item.category || "Listing"}
          </span>
          <span className="absolute bottom-12 left-3 max-w-[calc(100%-24px)] truncate rounded-full bg-white px-3 py-1 text-xs font-black text-[#4F46E5] shadow-lg ring-1 ring-[#C7D2FE] hover:bg-[#F5F3FF]">
            {item.title}
          </span>
          <span className="absolute bottom-3 left-3 rounded-full bg-white px-3 py-1 text-xs font-black text-[#4F46E5] shadow-lg ring-1 ring-[#C7D2FE] hover:bg-[#F5F3FF]">
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      {items.map(([icon, label]) => (
        <div key={label} className="flex min-h-16 items-center gap-3 rounded-[22px] border border-[#C7D2FE] bg-white px-4 py-3 shadow-sm hover:bg-[#F5F3FF]">
  ...
  return (
    <section className="my-8 hidden overflow-hidden rounded-[30px] bg-[#0F172A] text-white shadow-[0_24px_80px_rgba(7,21,46,0.22)] md:block">
      <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[0.65fr_1fr_0.65fr] lg:items-center">
        <div className="hidden h-36 overflow-hidden rounded-[26px] border border-white/10 bg-white/10 p-4 lg:block hover:bg-[#F5F3FF]">
          <div className="h-full rounded-[22px] bg-white p-4 text-[#4F46E5] shadow-2xl hover:bg-[#F5F3FF]">
        ...
        <div className="flex flex-wrap gap-3 lg:justify-end">
          <Link href="/listings" className="rounded-xl border border-white/20 bg-white px-4 py-2 text-xs font-black text-[#4F46E5] hover:bg-[#F5F3FF]">Browse listings</Link>
          <Link href="/sell/new" className="rounded-xl border border-white/20 bg-[#4F46E5] px-4 py-2 text-xs font-black text-white !text-white disabled:!text-white">Sell an item</Link>
        </div>
      </div>
    </section>
  );
...
export function EmptyMarketplaceState({ title, body, href, cta }: { title: string; body: string; href: string; cta: string }) {
  return (
    <div className="col-span-full rounded-[28px] border border-[#C7D2FE] bg-[#F6F8FB] p-8 text-center shadow-sm">
      <div className="mx-auto h-24 w-24 overflow-hidden rounded-[28px] border border-[#C7D2FE] bg-white hover:bg-[#F5F3FF]"><ProductPlaceholder kind="empty" /></div>

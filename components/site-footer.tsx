    ...
    <footer className="mt-10 border-t border-[#DDE7F4] bg-white pb-[calc(5.25rem+env(safe-area-inset-bottom))] md:pb-0">
      <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-11">
        <div className="grid gap-8 lg:grid-cols-[1.25fr_2fr_1fr] lg:items-start">
          <div>
            ...
            <p className="mt-4 max-w-sm text-sm leading-6 text-[#526173]">
              Australia&apos;s trust-first local marketplace. Buy now. Make
              offers. Arrange handover.
            </p>
                ...
                <div className="mt-3 grid gap-2.5">
                  {group.links.map((link) => (
                    <Link
                      key={link.href}
                      href={link.href}
                      className="text-sm font-semibold text-[#526173] transition hover:text-[#0B4DFF]"
                    >
                      {link.label}
                    </Link>
                  ))}
          ...
          <div className="rounded-[22px] border border-[#D7E2F1] bg-[#F7FAFF] p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-[#526173]">
              Get tips to list better and discover local deals.
            </p>
            <div className="mt-3 flex gap-2">
              <input
                aria-label="Email address"
                placeholder="Enter your email"
                className="bd-input min-w-0 flex-1 rounded-full bg-white text-sm"
              />
              <button
                type="button"
                className="bd-btn bd-btn-primary rounded-full px-4"
              >
                Subscribe
              </button>
            </div>
        ...
        <div className="mt-8 flex flex-col gap-3 border-t border-[#E6EDF7] pt-5 text-xs text-[#607089] md:flex-row md:items-center md:justify-between">
          ...
          <div>
            Bidra is a marketplace platform. Buyers and sellers arrange payment,
            pickup, postage and handover directly.
          </div>
        </div>
      </div>
    </footer>

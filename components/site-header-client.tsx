    ...
    <header className="border-b border-white/10 bg-[#0B0E11] text-white">
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
        ...
        <nav className="relative hidden items-center gap-4 text-sm md:flex">
          <Link href="/listings" className="hover:text-white/80" onClick={closeAll}>
            Browse
          </Link>
          <Link href="/sell" className="hover:text-white/80" onClick={closeAll}>
            Sell
          </Link>

          {isAuthed ? (
            <>
              <Link href="/messages" className="hover:text-white/80" onClick={closeAll}>
                Messages
              </Link>
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded-md border border-white/15 px-3 py-2 hover:bg-white/5"
              >
                {displayName}
              </button>
              ...
              {acctOpen ? (
                <div className="absolute right-0 top-12 z-50 w-56 rounded-md border border-white/10 bg-[#0B0E11] shadow-lg">
                  <div className="flex flex-col p-2 text-sm">
                    ...
                    <Link href="/dashboard" className="rounded px-3 py-2 hover:bg-white/5" onClick={closeAll}>
                      Dashboard
                    </Link>
                    <Link href="/orders" className="rounded px-3 py-2 hover:bg-white/5" onClick={closeAll}>
                      Orders
                    </Link>
                    <div className="my-1 border-t border-white/10" />
                    <Link href="/api/auth/signout" className="rounded px-3 py-2 hover:bg-white/5" onClick={closeAll}>
                      Sign out
                    </Link>
                  </div>
                </div>
            ...
            <>
              <Link href="/auth/login" className="hover:text-white/80" onClick={closeAll}>
                Sign in
              </Link>
              <Link href="/auth/register" className="hover:text-white/80" onClick={closeAll}>
                Create account
              </Link>
            </>
          )}
        ...
        <div className="flex items-center md:hidden">
          <button
            ...
            className="inline-flex items-center justify-center rounded-md border border-white/15 px-3 py-2 text-sm hover:bg-white/5"
          >
            Menu
          </button>
        </div>
            ...
            <div className="flex flex-col gap-3 text-sm">
              ...
              {isAuthed ? (
                <>
                  ...
                  <Link href="/api/auth/signout" onClick={closeAll}>Sign out</Link>
                </>
              ) : (
                <>

...
export default function SiteHeaderClient({ session, notificationCount = 0 }: { session?: SessionLike; notificationCount?: number }) {
  ...
  const searchInputClass = "h-11 w-full rounded-[14px] border border-white/18 bg-white/10 px-4 text-[14px] font-semibold text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] outline-none transition placeholder:text-white/55 focus:border-white/34 focus:ring-4 focus:ring-white/10";
  ...
  return (
    <header className="sticky top-0 z-[100] bg-[linear-gradient(135deg,#10061F_0%,#150A28_56%,#21103C_100%)] text-white shadow-[0_18px_55px_rgba(18,7,36,0.28)]" data-site-header>
      <div className="mx-auto hidden h-[82px] w-full max-w-[1440px] items-center gap-6 px-6 lg:flex xl:px-8">
        <Link href="/" className="flex h-16 w-[176px] shrink-0 items-center" aria-label="Bidra home">
          <BrandLogo priority tone="light" />
        </Link>

        <nav className="flex shrink-0 items-center gap-1.5 xl:gap-3" aria-label="Primary navigation">
          {DESKTOP_LINKS.map((link) => (
            <Link key={link.href} href={link.href} className="whitespace-nowrap rounded-full px-3 py-2.5 text-[13px] font-black text-white/86 transition hover:bg-white/10 hover:text-white">
              {link.label}
            </Link>
          ))}
        </nav>

        <SearchBar className="min-w-[18rem] max-w-[34rem] flex-1" inputClassName={searchInputClass} />
        ...
        <div className="flex shrink-0 items-center gap-3">
          ...
          {isAuthed ? (
            <div ref={desktopAccountRef} className="relative">
              <button type="button" onClick={(e) => { e.stopPropagation(); setDesktopAcctOpen(!desktopAcctOpen); }} className="inline-flex h-11 items-center gap-2 rounded-full border border-white/18 bg-white/8 px-4 text-sm font-black text-white transition hover:bg-white/12" aria-haspopup="menu" aria-expanded={desktopAcctOpen ? "true" : "false"}>
                {accountRoleLabel}{badge}
              </button>
              {desktopAcctOpen ? renderAccountMenu(() => setDesktopAcctOpen(false)) : null}
            </div>
          ) : (
            <Link href="/auth/login" className="inline-flex h-11 items-center rounded-full border border-white/18 bg-white/8 px-4 text-sm font-black text-white transition hover:bg-white/12">Sign in</Link>
          )}
          ...
          <Link href="/notifications" className="relative grid h-11 w-11 place-items-center rounded-full border border-white/18 bg-white/8 text-white transition hover:bg-white/12" aria-label="Notifications">
            <NotificationIcon />

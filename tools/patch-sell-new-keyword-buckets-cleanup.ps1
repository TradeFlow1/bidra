...
$content = $content.Replace('structure: details + pickup + payment note', 'structure: details + pickup + payment note')
$content = $content.Replace('Timed offers: I', 'Timed offers: I''ll review offers when the time ends and decide whether to proceed with the highest offer.')
$content = $content.Replace('Buy Now must be ', 'Buy Now must be ≥ starting offer.')
$content = $content.Replace('Your account isn', 'Your account isn''t age-verified yet. Please complete age verification to list items.')
$content = $content.Replace('Bidra accounts are 18+ only. You can browse publicly, but you can', 'Bidra accounts are 18+ only. You can browse publicly, but you can''t list or make offers.')
$content = $content.Replace('You can', 'You can''t create a listing right now.')
$content = $content.Replace('Add the basics ', 'Add the basics — title, description, category, condition, location, and pricing.')
$content = $content.Replace('Select a category', 'Select a category…')

[System.IO.File]::WriteAllText($patchPath, ($lines -join [Environment]::NewLine), (New-Object System.Text.UTF8Encoding($false)))
Write-Host "[OK] wrote $patchPath"
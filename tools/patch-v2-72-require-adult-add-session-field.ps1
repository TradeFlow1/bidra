...
$out = @(
  ...
  ''
  'type AnySession = any;'
  'export type RequireAdultResult = { ok: boolean; reason?: string; status?: number; session?: AnySession };'
  ''
  ...
  '    return { ok: false, reason: "Not authenticated", status: 401, session: s };'
  '  }'
  ''
  '  const age = (s as any).user?.age;'
  '  if (!age || age < 18) {'
  '    if (!session) { redirect("/account/restrictions"); }'
  '    return { ok: false, reason: "18+ restriction", status: 403, session: s };'
  '  }'
  ''

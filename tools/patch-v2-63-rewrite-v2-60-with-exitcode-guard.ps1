...
function Backup-File {
  param([string]$File)
  $bak = "$File.bak_v2_63_rewrite_v2_60"
...
$out = @(
  ...
  '  }'
  '  throw ''No successful ci.yml runs found to derive required check name.'''
  '}'
  ''
  ...
  '  }'
  '  foreach ($cr in $checks.check_runs) {'
  '    if ($cr.app -and $cr.app.slug -eq ''github-actions'') { return [string]$cr.name }'
  '  }'
  '  return [string]$checks.check_runs[0].name'
  '}'
  ''
  '$repoRoot = Find-RepoRoot'
  'Set-Location $repoRoot'

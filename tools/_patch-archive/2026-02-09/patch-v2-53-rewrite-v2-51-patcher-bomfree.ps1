...
function Backup-File {
  param([string]$File)
  $bak = "$File.bak_v2_53_rewrite_v2_51_patcher_bomfree"
...
$target = '.\tools\patch-v2-51-tighten-next-remotePatterns.ps1'
...
Write-Utf8NoBomText -File $target -Text $text
Write-Host ("Rewrote: {0} (UTF-8 no BOM)" -f $target)

Write-Host '== git diff -- tools/patch-v2-51-tighten-next-remotePatterns.ps1 =='
git diff -- tools/patch-v2-51-tighten-next-remotePatterns.ps1 | Out-Host

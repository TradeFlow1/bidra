...
$find = '(?s)\$results\.Add\(\(Replace-Exact\s+-RelPath\s+\$uiRel\s+-OldText\s+'[\s\S]*?Use 8\+ characters\.</p>'\s+-NewText\s+'[\s\S]*?\{passwordGuidanceText\(\)\}</p>'\s+-MinReplacements\s+1\s+-BackupTag\s+'bak_v2_46'\)\)\s+\|\s+Out-Null'
$replace = '$results.Add((Replace-Regex -RelPath $uiRel -Pattern '(?m)^(\s*)<p className=\{helper\}>Use 8\+ characters\.</p>' -Replacement '$1<p className={helper}>{passwordGuidanceText()}</p>' -MinReplacements 1 -BackupTag 'bak_v2_46')) | Out-Null'

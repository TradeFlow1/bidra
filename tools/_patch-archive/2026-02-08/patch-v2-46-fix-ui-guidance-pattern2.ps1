...
# Replace only the UI helper Replace-Exact call so it does NOT depend on indentation
...
$targetLine = '$results.Add((Replace-Exact -RelPath $uiRel -OldText '<p className={helper}>Use 8+ characters.</p>' -NewText '<p className={helper}>{passwordGuidanceText()}</p>' -MinReplacements 1 -BackupTag 'bak_v2_46')) | Out-Null'
...
if ($Apply) {
  if ($PSCmdlet.ShouldProcess($patchRel, "Make UI helper Replace-Exact indentation-tolerant")) {
    $bak = $patchPath + '.bak_v2_46_uipat2'

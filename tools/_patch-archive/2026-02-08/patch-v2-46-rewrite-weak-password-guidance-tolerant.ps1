...
$P.Add('  param([string]$RelPath,[string]$Pattern,[string]$Replacement,[int]$MinReplacements,[string]$BackupTag,[string]$AlreadyAppliedPattern)') | Out-Null
...
if ($Apply) {
  if ($PSCmdlet.ShouldProcess($patchRel, "Rewrite patch script (tolerant)")) {
    if (Test-Path -LiteralPath $patchPath) {
      $bak = $patchPath + '.bak_v2_46_rewrite_tolerant'
  ...
  Write-Host ("Dry-run: would rewrite " + $patchRel + " (tolerant)")
}

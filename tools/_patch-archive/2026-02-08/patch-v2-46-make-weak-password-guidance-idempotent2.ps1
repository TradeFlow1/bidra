...
# Replace Replace-Regex function with an idempotent version
$funcFind = '(?s)function\s+Replace-Regex\s*\{\s*param\(\[string\]\$RelPath,\[string\]\$Pattern,\[string\]\$Replacement,\[int\]\$MinReplacements,\[string\]\$BackupTag\)\s*[\s\S]*?\r?\n\}'
...
# Add -AlreadyAppliedPattern to the 3 Replace-Regex callsites that fail on re-run
$changes = 0

$p1 = '(Replace-Regex\s+-RelPath\s+\$regRel[\s\S]*?)(-MinReplacements\s+1\s+-BackupTag\s+''bak_v2_46'')'
if ([regex]::IsMatch($text, $p1)) { $text = [regex]::Replace($text, $p1, '$1-AlreadyAppliedPattern ''checkPasswordPolicy\(\s*passwordStr\s*\)'' $2', 1); $changes++ }

$p2 = '(Replace-Regex\s+-RelPath\s+\$resetRel[\s\S]*?)(-MinReplacements\s+1\s+-BackupTag\s+''bak_v2_46'')'
if ([regex]::IsMatch($text, $p2)) { $text = [regex]::Replace($text, $p2, '$1-AlreadyAppliedPattern ''checkPasswordPolicy\(\s*newPassword\s*\)'' $2', 1); $changes++ }

$p3 = '(Replace-Regex\s+-RelPath\s+\$fixRel[\s\S]*?)(-MinReplacements\s+1\s+-BackupTag\s+''bak_v2_46'')'
if ([regex]::IsMatch($text, $p3)) { $text = [regex]::Replace($text, $p3, '$1-AlreadyAppliedPattern ''"Id"\\s*:\\s*46[\\s\\S]*?"Done"\\s*:\\s*true'' $2', 1); $changes++ }
...
if ($Apply) {
  if ($PSCmdlet.ShouldProcess($patchRel, "Make patch idempotent for re-runs")) {
    $bak = $patchPath + '.bak_v2_46_idempotent2'

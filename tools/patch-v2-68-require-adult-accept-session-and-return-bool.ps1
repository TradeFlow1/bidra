...
$out = @(
  ...
  'type AnySession = any;'
  ''
  'export async function requireAdult(session?: AnySession): Promise<boolean> {'
  '  const s = session || (await auth());'
  ''
  '  if (!s?.user) {'
  '    // when enforcing (no session supplied), send to login'
  ...
  ''
  '  // Only enforce redirects when this helper is used as a guard (no session passed)'
  '  if (!session && !adult) {'
